...
class Storage:
    ...
    def create_concept(self, id: str, name: str, description: str, 
                       category: str, embedding_id: Optional[str] = None) -> bool:
        """创建概念"""
        conn = self._get_connection()
        cursor = conn.cursor()
        try:
    ...
    def list_concepts(self, category: Optional[str] = None, 
        ...
        if category:
            cursor.execute(
                "SELECT * FROM concepts WHERE category = ? LIMIT ?",
                (category, limit)
            )
        else:
    ...
    def create_insight(self, id: str, title: str, content: str,
                       trigger_content: Optional[str] = None,
                       source: Optional[str] = None,
                       related_concepts: Optional[List[str]] = None,
                       action_items: Optional[List[str]] = None) -> bool:
        """创建 Insight"""
        import json
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute(
            '''INSERT INTO insights 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?)''',
            (id, title, trigger_content, source, content,
